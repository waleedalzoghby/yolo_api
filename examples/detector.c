...
/* workaround for opencv 3.x when compiling in debug mode */
#ifdef OPENCV
#include "math.h"
...
static void print_dollar_detections(FILE *fp, char *id, detection *dets, int total, int classes, int w, int h)
{
    int i, j;
    ...
    for (i = 0; i < total; ++i) {
        ...
        for(j = 0; j < classes; ++j){
            int class = j;
            if (dets[i].prob[class]) fprintf(fp, "%s,%f,%f,%f,%f,%f,%d\n", id, bx, by, bw, bh, dets[i].prob[class], class);
        }
    }
}

static void print_voc_detections(FILE **fps, char *id, detection *dets, int total, int classes, int w, int h)
{
    int i, j;
    for(i = 0; i < total; ++i){
...
void validate_detector_flip(const char *datacfg, const char *cfgfile, const char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            if (coco){
                ...
                print_voc_detections(fps, id, dets, num, classes, w, h);
            }
    ...
    for(i = nthreads; i < m+nthreads; i += nthreads){
        ...
        for(t = 0; t < nthreads && i+t-nthreads < m; ++t){
            ...
            if (coco){
                ...
                print_dollar_detections(fp, id, dets, nboxes, classes, w, h);
            } else {
                print_voc_detections(fps, id, dets, nboxes, classes, w, h);
            }
...
void validate_detector_recall(const char *datacfg, const char *cfgfile, const char *weightfile)
{
    list *options = read_data_cfg(datacfg);
...
void validate_detector_PRcurve(const char *datacfg, const char *cfgfile, const char *weightfile)
{
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/voc.2007.test");
    ...
    for(float thresh = 0; thresh < 1; thresh = thresh + 0.01){
        ...
        for(i = 0; i < m; ++i){
            ...
            int *used_det = (int *) calloc(nboxes, sizeof(int));
            if (used_det == NULL) {
